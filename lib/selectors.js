...
const graphical_selectorPart_component = 'image|img|video|hr|picture|photo|icon|i$|shape|before$|after$|input|figure|hr$|svg|line|logo|frame|button|select|textarea';
const graphical_selectorPart_notGraphical = 'image|img|video|hr|picture|photo|icon|i$|shape|before$|after$|figure|hr$|svg|line|logo|frame|button|input|select$|textarea';
...
export const component_selectors = new RegExp('^(?!& )(?!.*__)([.]|\\[[a-z0-9-_]*="?)(?!.*(?:'+graphical_selectorPart_component+'))[a-zA-Z0-9-_]+("?\\])?$');
export const notGraphical_selectors = new RegExp('^(?!.*(?:'+graphical_selectorPart_notGraphical+')).*$');
